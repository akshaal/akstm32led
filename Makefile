...
OBJS_DBG    = $(addprefix tmp-dbg/,$(SRCS:.c=.o)) ${OBJS_3RD}
...
all: tmp/$(TARGET)-opt.bin tmp/$(TARGET)-dbg.elf | dirs
...
tmp/$(TARGET)-dbg.elf: $(OBJS_DBG) | dirs
	$(CC) $(CFLAGS) $(LDFLAGS_DBG) $(LDFLAGS) src-3rd/${STARTUP_S} $^ -o tmp/$(TARGET)-dbg.elf
	$(OBJDUMP) -St tmp/$(TARGET)-dbg.elf >tmp/$(TARGET)-dbg.lst
	$(SIZE) tmp/$(TARGET)-dbg.elf
